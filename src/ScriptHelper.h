...
static std::vector<MonoClass*> GetAssemblyClassList(MonoImage * image)
{
...
static bool compile_script(const std::vector<std::string>& scripts, const std::string& dll_reference = "")
{
    // Get paths
    ...
    std::string outName = "temp/" + p.filename().replace_extension(".dll").string();
    command += " -out:" + outName;
...
static bool compareTimestamps(const std::string& left, const std::string& right)
{
...
static MonoAssembly* compile_and_load_assembly(MonoDomain* domain, const std::vector<std::string>& scripts, bool is_script = true)
{
        // Open assembly
    std::filesystem::path p(scripts.at(0));
    const std::string outName = "temp/" + p.filename().replace_extension(".dll").string();
    ...
    if (!result)
    {
        ...
        if (is_script)
        {
            const std::string callbacks = "temp/Engine.dll";
